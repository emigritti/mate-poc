"""Contextual Retrieval (ADR-X4) — ingestion-platform adapter.

Mirrors integration-agent/services/contextual_retrieval_service.py but operates
on CanonicalChunk objects instead of DoclingChunk.

Prepends a situating annotation to each chunk text before it is embedded.
Provider selection:
  - Claude (default) — prompt caching: system + doc cached once per batch.
  - Ollama (fallback) — offline-capable, no API key required.

Compliance (CLAUDE.md §1):
  - Chunk text is sent to Claude API → only synthetic / public / Accenture-Internal data.
"""
...
async def _call_claude_for_context(client, doc_text: str, chunk_text: str) -> str:
...
async def add_context_to_chunks(
    ...
    """Prepend situating annotations to each chunk before embedding.

    Returns a new list of CanonicalChunk objects with enriched .text fields.
    Falls back to original chunks on any individual failure (graceful degradation).
    """
    ...
    for c in chunks:
        ...
        try:
            ...
            logger.warning(
                "[Ctx-Retrieval] failed for chunk %d source=%s (%s) — keeping original.",
                c.index, c.source_code, exc,
            )
        if situating:
            out.append(c.model_copy(update={"text": _wrap(situating, c.text)}))
        else:
            out.append(c)
    return out
